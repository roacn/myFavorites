...
update: 2021.7.22 20:30
* 修复了助力活动不存在、增加了随机UA（如果未定义ua则启用随机UA）
* 新增推送
* 修复0点不能开团
* 兼容pin为中文转码编码
* 增加判断，如无法达成下一级助力奖励，即停止对该账号助力，跳转助力下一账号。
'''
...
def getShareCode(ck):
    ...
    try:
        ...
        try:
            ...
            progressAlready = int(jsonp['data']['beanCountProgress']['progressAlready'])
            progressNotYet = int(jsonp['data']['beanCountProgress']['progressNotYet'])
        except:
            if aNum < 5:
                aNum += 1
                ...
                progressNotYet = 0
        aNum = 0
        return groupCode, shareCode, sumBeanNumStr, activityId, progressAlready, progressNotYet
    except Exception as e:
...
def start():
    ...
    length_cookiesList = len(cookiesList)
    # 助力次数
    # if "bean_limit" in os.environ:
    #     bean_limit_num = int(os.environ["bean_limit"]) if int(os.environ["bean_limit"]) > 0 else 9999
    # else:
    #     bean_limit_num = bean_limit
    i = 0
    length_qjd_zlzh = len(qjd_zlzh)
    while i < length_qjd_zlzh:
        try:
            ckname = qjd_zlzh[i]
            ...
            try:
                ...
                continue
        print(f"\n====== 开始助力账号【{userNameList[int(ckNum)]}】======\n")
        groupCode, shareCode, sumBeanNumStr, activityId, progressAlready, progressNotYet = getShareCode(cookiesList[ckNum])
        if groupCode == 0:
            msg(f"## {userNameList[int(ckNum)]}  获取互助码失败。请手动分享后再试~ 或建议早上再跑。")
            continue
        print(f'当前获得京豆助力{progressAlready}人次，下一阶段需要人数：{progressNotYet}人次！')
        u = 0
        v = 0
        if progressAlready + progressNotYet < length_cookiesList:
            for j in cookiesList:
                if v < progressNotYet:
                    if j == cookiesList[ckNum]:
                        u += 1
                        continue
                    result = helpCode(j, groupCode, shareCode, userNameList[u], u+1, userNameList[int(ckNum)], activityId)
                    time.sleep(sleepNum)
                    if result:
                        break
                    u += 1
                    ...
                    break
            newProgressAlready = getShareCode(cookiesList[ckNum])[4]
            if newProgressAlready == progressAlready:
                i += 1
        else:
            i += 1
            print(f'目前队伍无法满足需求：{progressNotYet}人次，跳转下一助力！')
        groupCode, shareCode, sumBeanNumStr, activityId, progressAlready, progressNotYet = getShareCode(cookiesList[ckNum])
        userCount[f'{userNameList[ckNum]}'] = sumBeanNumStr
    print("\n------------------------------")
    for i in userCount.keys():
        msg(f"账号【{i}】已抢京豆: {userCount[i]}")
    msg(f"## 今日累计获得 {sumBeanNumStr} 京豆")
    try:
