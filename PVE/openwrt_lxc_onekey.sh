...
update_CT_Templates(){
    ...
    TIME y "下载OpenWrt固件"
    TIME g "获取固件API信息..."
    wget -q --timeout=5 --tries=1 --show-progress ${Github_API} -O ${Download_Path}/Github_Tags
    if [[ $? -ne 0 ]];then
        wget -q --timeout=5 --tries=1 --show-progress https://pd.zwc365.com/${Release_Download_URL}/Github_Tags -O ${Download_Path}/Github_Tags
        if [[ $? -ne 0 ]];then
            wget -q --timeout=5 --tries=1 --show-progress https://ghproxy.com/${Release_Download_URL}/Github_Tags -O ${Download_Path}/Github_Tags
            if [[ $? -ne 0 ]];then
    ...
    wget -q --timeout=5 --tries=1 --show-progress ${Release_Download_URL}/$(cat ${Download_Path}/DOWNLOAD_URL) -O ${Download_Path}/openwrt.rootfs.img.gz
    if [[ $? -ne 0 ]];then
        wget -q --timeout=5 --tries=1 --show-progress https://pd.zwc365.com/${Release_Download_URL}/$(cat ${Download_Path}/DOWNLOAD_URL) -O ${Download_Path}/openwrt.rootfs.img.gz
        if [[ $? -ne 0 ]];then
            wget -q --timeout=5 --tries=1 --show-progress https://ghproxy.com/${Release_Download_URL}/$(cat ${Download_Path}/DOWNLOAD_URL) -O ${Download_Path}/openwrt.rootfs.img.gz
            if [[ $? -ne 0 ]];then
